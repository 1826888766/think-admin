...
class Template  extends ThinkTemplate
{
    ...
    private function compiler(string &$content, string $cacheFile): void
    {
        ...
        if ($this->config['layout_on']) {
            if (false !== strpos($content, '{__NOLAYOUT__}')) {
                ...
                // 读取布局模板
                // 获取视图根目录
                $file = $this->parseTemplate($this->config['layout_name']);
                $layoutFile = $this->parseTemplateFile($file);
    ...
    private function parseTemplate($file)
    {
        if (strpos($file, '@')) {
            // 跨模块调用
            list($app, $file) = explode('@', $file);
        }
        $file   = str_replace(['/', ':'], $this->config['view_depr'], $file);
        if (isset($app)) {
            $view     = $this->config['view_dir_name'];
            $viewPath = app()->getBasePath() . $app . DIRECTORY_SEPARATOR . $view . DIRECTORY_SEPARATOR;

            if (is_dir($viewPath)) {
                $path = $viewPath;
            } else {
                $path = app()->getRootPath() . $view . DIRECTORY_SEPARATOR . $app . DIRECTORY_SEPARATOR;
            }
            $file = $path . ltrim($file, '/') . '.' . ltrim($this->config['view_suffix'], '.');
        }
        return $file;
    }
